...
When this option is enabled, every generated hook also accepts an optional `overrideKey` callback as its last argument. It receives the autogenerated key and returns the key that is actually passed to `useSWR()` / `useSWRInfinite()`, so you can override it entirely or extend it while keeping the autogenerated part intact:

```ts
// Override the key completely
sdk.useMyQuery(variables, undefined, () => ['MyCustomKey'])

// Extend the autogenerated key with extra data
sdk.useMyQuery(variables, undefined, (key) => [...key, currentUserId])
```
