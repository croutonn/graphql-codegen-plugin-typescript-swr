...
// Extracts the key-computation expression generated for `useFeed` (the
// `overrideKey ? overrideKey(genKey(...)) : genKey(...)` ternary) together
// with the `genKey` helper it closes over, so `overrideKey`'s effect on the
// key actually passed to `useSWR` can be exercised at runtime.
const evaluateFeedKeyExpression = (
  output: string
): ((
  variables: unknown,
  overrideKey?: (key: unknown[]) => unknown[]
) => unknown[]) => {
  const genKeyMatch = output.match(/ {2}const genKey = (.*);\n/)
  const exprMatch = output.match(
    /return useSWR<FeedQuery, ClientError>\(([\s\S]*?), \(\) => sdk\.feed\(variables\)/
  )
  if (!genKeyMatch || !exprMatch) {
    throw new Error('genKey or useFeed key expression not found in generated output')
  }
  const { outputText } = ts.transpileModule(
    `const genKey = ${genKeyMatch[1]};\nmodule.exports = (variables, overrideKey) => (${exprMatch[1]});`,
    { compilerOptions: { module: ts.ModuleKind.CommonJS } }
  )
  const sandboxModule = { exports: {} }
  vm.runInNewContext(outputText, {
  ...
  return sandboxModule.exports as ReturnType<typeof evaluateFeedKeyExpression>
}
...
async function test() {
  ...
  it('Should pass the autogenerated key through unchanged when `overrideKey` is not given', async () => {
    const config: PluginsConfig = { autogenSWRKey: true }
    const docs = [{ location: '', document: basicDoc }]
    ...
    const output = await validate(content, config, docs, schema, basicUsage)
    const computeKey = evaluateFeedKeyExpression(output)

    expect(computeKey({ v: '1' }, undefined)).toEqual(['Feed', '1'])
  })

  it('Should let `overrideKey` extend the autogenerated key while keeping it intact', async () => {
    const config: PluginsConfig = { autogenSWRKey: true }
    const docs = [{ location: '', document: basicDoc }]
    ...
    const result = computeKey({ v: '1' }, (key) => [...key, 'extra'])

    expect(result).toEqual(['Feed', '1', 'extra'])
  })

  it('Should let `overrideKey` fully replace the autogenerated key', async () => {
    const config: PluginsConfig = { autogenSWRKey: true }
    const docs = [{ location: '', document: basicDoc }]
    ...
    const result = computeKey({ v: '1' }, () => ['CustomKey'])
